...
**Care-assistant inference** runs in the Chat Service when an authorised orchestrator requests completions. **Clinical risk evaluation** runs in Care Episode after the assistant turn is persisted. Both use a synchronous orchestration path on the patient chat write path with an OpenAI-compatible HTTP chat-completions API. The specific vendor is an operator choice — not an architectural constant.
...
4. **OpenAI-compatible integration shape.** Chat integrates for care-assistant completions; Care Episode integrates for clinical risk evaluation — both through an OpenAI-compatible chat-completions HTTP API so providers can be changed without service code changes.
