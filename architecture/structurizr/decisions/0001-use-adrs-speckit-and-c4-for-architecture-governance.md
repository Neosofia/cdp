# 1. Use ADRs, Spec Kit, and C4 Modeling for Specification and Architecture Governance
...
2. **Spec Kit** — All feature work begins with a structured specification and planning workflow (`specify → clarify → plan → tasks → implement`). Each service has a canonical `spec.md` under `specs/NNN-service-name/`. The constitution (`/.specify/memory/constitution.md`) encodes non-negotiable platform principles that all specs must comply with.
