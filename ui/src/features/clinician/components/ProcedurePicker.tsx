...
export default function ProcedurePicker({
  ...
  const { entries, typeLabels, loading, error } = useProcedureCatalog(token, activeActor);
  const selected = selectedId ? entries.find((entry) => entry.id === selectedId) : undefined;

  const grouped = useMemo(
    () => groupProceduresBySpecialty(searchProcedureCatalog(entries, filter, typeLabels)),
    [entries, filter, typeLabels],
  );
  ...
  return (
    <div className="space-y-3">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className={formStyles.pickerSelectedSubClass}>
                {typeLabels[selected.procedureType]}
                {' · '}
      ...
      >
        {loading ? (
          <p className="px-3 py-4 text-sm text-slate-500 text-center">Loading procedures…</p>
        ) : error ? (
                    ...
                    <li key={entry.id}>
                      ...
                      >
                        ...
                        <span className={formStyles.pickerItemMetaClass}>
                          {typeLabels[entry.procedureType]}
                          {' · '}
