...
A distributed care platform fails in ways no single service log can explain: the notification relay retried, enrollment stalled, or chat activity dropped while error rates looked fine. Without a **platform-wide observability layer**, each team would instrument differently, growth numbers would be argued per service, and nobody could reconstruct an end-to-end trace from patient message to alert attempt. Operators would fly blind; product and executive stakeholders could not report adoption trends; and Information Security could not independently verify that PHI never leaked into broadly accessible dashboards.

Platform Observability is the **shared contract** for structured events, log aggregation, trace correlation, and read-only dashboards — always without patient-identifiable content in events, metric labels, or dashboard rows. Services emit facts; a **central log store** holds them for investigation and aggregation in staging and production.

## Two dashboards, two audiences

| Dashboard | Audience | Purpose |
|-----------|----------|---------|
| **Reliability (DORA)** | DevOps, on-call engineers | Error rate, latency, unhandled errors, auth failures — service health and release confidence |
...
**Service authors** need one supported emission path, schema validation at write time, and registerable event types without redeploying the central aggregator.
...
- **FR-001**: All platform services emit structured log events through the standard log plugin. Direct writes to the central log store outside the standard emission path are not supported.
...
- **FR-004**: The central store retains raw log events for operator investigation. Business engagement aggregates are derived from structured service events. Retention must support multi-week and quarterly trend reporting.

- **FR-005**: The **product engagement** dashboard publishes **business measures** including, at minimum:

  | Measure | What it answers |
  |---------|-----------------|
  | Patient enrollments | How many patients started a new care episode? |
  | Successful logins | How many people used the product? |
  | Patient chat turns | How many patient messages received an AI reply? |
  | Care-assistant replies | How much AI-assisted care was delivered? |
  | Clinician messages | How often did the care team participate in chat? |
  | Clinician response rate | What share of patient turns got a human follow-up? |
  | Clinical escalations | How many high-severity patient chat turns resulted in a clinician alert email? |
  | Recoveries closed | How many care windows ended? |

  Enrollments count **new care episodes opened**, not registry sync on login. Escalations count **successful clinician alert handoff**, not all notification email traffic. Panels show **counts** and **period totals** (not latency). Default time ranges support weekly, monthly, and quarterly review.

- **FR-006**: The **DORA** dashboard publishes **reliability measures** including, at minimum: request error rate, p95 latency, unhandled application errors, authentication failure rate, and request rate by service. A health summary covers per-service reachability.

- **FR-007**: When a configured **reliability** measure breaches its threshold, the alerting platform raises an alert with measure name, current value, threshold, trace correlator when available, and runbook link.

- **FR-008**: Every event carries trace and span identifiers where the request path supports them; operators can reconstruct the ordered event chain for a given trace in the log investigation UI.
...
- **FR-010**: The aggregator detects service silence: when a service that historically emits events produces none for a configurable quiet-period threshold during expected active hours, a silence alert fires on the reliability dashboard.

- **FR-011**: New service event types are registerable without redeploying the central log store.
...
- **OR-001**: Platform operators can deploy dashboards, log ingest, alert rules, and retention policies independently of individual service releases. Run and query details live in the infrastructure repository.

- **OR-002**: Operators can measure alert propagation time, trace reconstruction, retention compliance, and multi-tenant query isolation through automated checks in staging and production.
...
- Dashboard operations: [infrastructure/public-cloud/grafana/README.md](https://github.com/Neosofia/infrastructure/blob/main/public-cloud/grafana/README.md)
