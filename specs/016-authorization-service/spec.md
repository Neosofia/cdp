...
Identity (who the caller is) is established by the Authentication Service (014-authentication-service) and encoded in the JWT. The Authorization Service consumes those identity claims as input to Cedar policy evaluation and returns a `Allow` or `Deny` decision. No service in the platform implements its own role-string checks — all access control decisions are delegated here.

The policy engine is [Cedar](https://www.cedarpolicy.com/), backed by [AWS Verified Permissions](https://aws.amazon.com/verified-permissions/) (the managed Cedar service). Cedar policies are stored in Verified Permissions and are the authoritative source of truth for what each principal may do. The service exposes a thin HTTP wrapper that other platform services call rather than integrating with the AWS SDK directly.
...
5. **Given** a machine service token for `svc-notification`, **When** it requests an action not covered by any Cedar `permit` policy for `cdp::Service::"svc-notification"`, **Then** a `Deny` decision is returned (Cedar default-deny)
...
1. **Given** a new Cedar policy is published to the Verified Permissions policy store, **When** the Authorization Service next evaluates a request matching that policy, **Then** the new policy is applied without requiring a service deployment
2. **Given** a Cedar policy is deleted or disabled, **When** a previously-allowed request is evaluated, **Then** a `Deny` decision is returned
...
- What happens if the JWT claims are missing a field the Cedar policy expects? The Authorization Service treats the missing attribute as an empty/unknown value; Cedar's `forbid` default applies — the request is denied.
- What happens if AWS Verified Permissions is unavailable? The service MUST fail closed — return `Deny` with reason `authz-service-unavailable` within a configurable timeout (default: 500 ms). The API gateway treats this identically to an explicit `Deny`.
- What happens when a policy conflict exists (multiple applicable policies)? Cedar's evaluation semantics apply: any applicable `forbid` policy overrides all `permit` policies. The service does not add its own override logic on top of Cedar's semantics.
- What happens when a new resource type is introduced before its Cedar policy exists? No `permit` policy matches → Cedar defaults to `Deny`. New resource types are dark until a policy is published.
...
- **FR-001**: The service MUST expose a `POST /authorize` endpoint that accepts a principal (JWT claims object), an action (namespaced Cedar action string), and a resource (namespaced Cedar entity identifier), and returns a Cedar `Allow` or `Deny` decision with a machine-readable reason code
...
- **FR-003**: All policy evaluation MUST be delegated to AWS Verified Permissions; the service MUST NOT implement its own policy evaluation logic
- **FR-004**: The service MUST return a `Deny` decision if AWS Verified Permissions is unavailable, does not respond within 500 ms, or returns an unexpected error (fail-closed)
...
- **FR-007**: The service MUST expose a health endpoint (`GET /health`) that reports Verified Permissions reachability; downstream callers (gateway, services) use this for circuit-breaking
- **FR-008**: Cedar policies MUST be managed exclusively in the AWS Verified Permissions policy store; no policies are hardcoded in service source code
- **FR-009**: The service MUST be callable only from within the platform's private network (VPC); the authorization endpoint is not publicly routable
...
- **NFR-002**: The service MUST be horizontally scalable; no in-process state; all policy state is in Verified Permissions
- **NFR-003**: Policy changes in Verified Permissions MUST propagate to decision evaluation within 10 seconds of publication (eventual consistency window for Verified Permissions is acceptable within this bound)
...
- **SC-002**: Service returns `Deny` within 500 ms when Verified Permissions is unreachable (fail-closed verified by chaos test)
...
- AWS Verified Permissions is the managed Cedar policy store; all Cedar policies are authored and version-controlled outside of this service's codebase (in a dedicated `policies/` repository or Terraform module) and deployed independently of service code
- The Authentication Service (014-authentication-service) is the sole issuer of JWTs; the Authorization Service trusts JWT claims only after the caller has been authenticated
- The Authorization Service does not validate JWT signatures; callers are responsible for providing pre-validated claims (the API gateway validates via 014 introspection before calling 017)
- Cedar policy authoring, review, and deployment is the responsibility of the platform security team; this service provides the runtime evaluation endpoint only
...
- AWS IAM permissions for the service's execution role are scoped to `verifiedpermissions:IsAuthorized` and `verifiedpermissions:IsAuthorizedWithToken` only; no policy management IAM permissions are granted to the service runtime

## Out of Scope

- Cedar policy authoring tooling and CI/CD pipeline for policies (platform infra concern)
- Human-facing policy management UI
- Audit log querying UI (CloudWatch Log Insights is sufficient for this phase)
- Relationship-based access control (ReBAC) patterns beyond what Cedar natively supports
