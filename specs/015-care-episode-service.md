...
The Chat Service depends on this service for a patient’s active episode before opening a new interaction -- every message needs a `care_episode_id`. When multiple episodes overlap, the most recently opened active episode wins until multi-episode selection is productised. Episodes close automatically when the care window expires (scheduled job) or explicitly when a clinician or admin closes them; closure emits lifecycle events with identifiers only so subscribers can react without PHI in the bus payload.

**Initial version:** a **chat-session bootstrap** aggregator endpoint (FR-015) resolves the active episode, builds the interaction context snapshot, and creates the chat interaction via Chat — so channel clients do not orchestrate those steps. That context feeds the synchronous care assistant, including clinical risk evaluation on each patient turn ([001](https://github.com/Neosofia/cdp/blob/main/specs/001-chat-service.md) FR-011, [010](https://github.com/Neosofia/cdp/blob/main/specs/010-ai-agent-service.md)).
...
- **FR-015**: A chat-session bootstrap API resolves the patient's active care episode ([FR-004](https://github.com/Neosofia/cdp/blob/main/specs/015-care-episode-service.md)), derives the interaction context snapshot from authoritative episode and session data, and creates a chat interaction via the Chat Service internal API. Clients receive `care_episode_uuid`, `chat_interaction_uuid`, and any non-sensitive metadata needed to continue messaging. **Initial version:** tracked in [Neosofia/care-episode#1](https://github.com/Neosofia/care-episode/issues/1). Context supplied through this path feeds the synchronous care assistant, including clinical risk evaluation on each patient turn ([001](https://github.com/Neosofia/cdp/blob/main/specs/001-chat-service.md) FR-011, [010](https://github.com/Neosofia/cdp/blob/main/specs/010-ai-agent-service.md)).
