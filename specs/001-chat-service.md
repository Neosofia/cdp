...
- **FR-004**: A chat interaction ends when the patient explicitly closes via the post-chat feedback window or after fifteen minutes of inactivity. The end reason -- patient closed or inactivity timeout -- is recorded, and an interaction-end signal is published containing only identifiers and end reason, no message content, so the deidentification pipeline can fetch the full log from storage. **Initial version:** deferred together with the deidentification pipeline ([002](https://github.com/Neosofia/cdp/blob/main/specs/002-deidentification-pipeline.md)) and clean chat store ([003](https://github.com/Neosofia/cdp/blob/main/specs/003-clean-chat-service.md)); open-ended interactions until that bundle ships.
...
- **FR-008**: **Dropped (v1).** A configurable per-patient rate limit on inbound messages is out of scope for the initial version; platform baseline abuse protection applies until channel adapters ship at scale ([000-platform-baseline.md](https://github.com/Neosofia/cdp/blob/main/specs/000-platform-baseline.md)).

- **FR-009**: Duplicate submissions, empty or whitespace-only payloads, and oversize messages are handled deterministically: invalid payloads are rejected with clear errors without partial side effects. Duplicate-submission deduplication and idempotency keys are out of scope for the initial version.

- **FR-010**: Structured lifecycle signals are emitted for significant state changes so async pipelines can react without polling chat storage. Signals never contain raw message content or PHI. **Initial version:** deferred with FR-004 and the deidentification bundle ([002](https://github.com/Neosofia/cdp/blob/main/specs/002-deidentification-pipeline.md), [003](https://github.com/Neosofia/cdp/blob/main/specs/003-clean-chat-service.md)); operator dashboards rely on OR-001 until that bundle ships.

- **FR-011**: For every inbound patient message, the care assistant performs clinical risk evaluation in the same turn as the conversational reply, producing a binary intervention signal per [010-ai-agent-service.md](https://github.com/Neosofia/cdp/blob/main/specs/010-ai-agent-service.md). Evaluation uses conversation context (including authoritative episode context when the patient started the chat, per [015](https://github.com/Neosofia/cdp/blob/main/specs/015-care-episode-service.md) FR-015) and full thread history. When intervention is required, the service triggers the Notification Service escalation path within the platform time budget. A risk evaluation record is stored for every message without retaining message text in that record. **Initial version:** evaluation runs in the same turn as the reply; a separate async risk service is deferred until scale requires it ([010](https://github.com/Neosofia/cdp/blob/main/specs/010-ai-agent-service.md) future path).
...
- **OR-001**: Logs support **measuring** chat ingestion and access without raw message content or other PHI. At minimum:

  - Classifying request outcomes and errors by endpoint
  - Attributing request duration by endpoint
  - Counting inbound messages written, completion turns, and interaction opens

  Tenant and channel breakdown must not use message body or PHI. See [011-operational-metrics.md](https://github.com/Neosofia/cdp/blob/main/specs/011-operational-metrics.md) for the authoritative measurement model.
